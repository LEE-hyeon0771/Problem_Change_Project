# 초기 설계 스펙 — FastAPI 기반 “영어 지문 → 유형별 문제 변형” 서비스 (10 Agents / 10 APIs, 고급 설계 포함)

> **보관 문서입니다.** 현재 코드 기준의 유효한 가이드는 루트의 `CLAUDE.md`이며, 충돌 시 `CLAUDE.md`가 우선합니다.
> 이 문서는 초기 설계 의도를 남겨두기 위한 기록입니다.
...
backend/
  ...
  prompts/
    base_system.md
    analysis.md   (현재 미사용 — 지문 분석은 BaseAgent.analyze() 가 파이썬으로 계산)
    title.md
    long.md
    summary.md
    insertion.md
...
`backend/core/config.py`에서 pydantic-settings로 로딩.
...
> 실제 구현에서는 `backend/prompts/*.md`로 저장하고, `{passage}`, `{difficulty}`, `{seed}`, `{style}` 같은 변수로 포맷.
