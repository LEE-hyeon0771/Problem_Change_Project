# 설계 업데이트 스펙 — 주제/함축의미추론 2유형 추가 + 품질/안정성 강화 지침

> **보관 문서입니다.** 현재 코드 기준의 유효한 가이드는 루트의 `CLAUDE.md`이며, 충돌 시 `CLAUDE.md`가 우선합니다.
> 이 문서는 당시 변경 의도를 남겨두기 위한 기록입니다.

이 문서는 기존 `docs/spec-initial.md`(및 저장소에 포함되지 않은 advanced 설계 메모) 위에 **추가 구현해야 할 변경 사항만** 정리한 “업데이트 지시서”다.  
...
### 2.3 구현해야 할 공통 유틸(권장 위치: `backend/toolkit/text.py`)
...
### 4.4 API 스키마(`backend/schemas/topic.py`)
...
### 5.5 API 스키마(`backend/schemas/implicit.py`)
