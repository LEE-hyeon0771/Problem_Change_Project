...
- easy: common idiom or transparent metaphor; the correct paraphrase is near-literal.
- mid: the expression's literal reading is plausible but wrong; one distractor exploits it.
- hard: the expression carries tone or stance (irony, concession, hedging). The literal
  reading AND two topical readings must both look defensible. Only context settles it.
- hard: no option may reuse the underlined expression's own words (L1).
...
Span selection (this type lives or dies on the span):
- The span must be NON-LITERAL: an idiom, a metaphor, a metonymy, or a stance-bearing
  phrase (irony, concession, understatement). A plain descriptive phrase has no implied
  meaning to ask about.
- The span's meaning must be recoverable from context, not from prior idiom knowledge.
  If a student must already know the idiom, the item tests vocabulary, not inference.
- Prefer spans where the literal reading is coherent but wrong — that literal reading
  becomes your strongest distractor.

Distractor recipes for this type:
- the LITERAL reading of the span (always include at mid/hard)
- `polarity_flip`: the right domain, the opposite attitude
- `scope_broad`: a general truth the metaphor gestures at but does not assert here
- `keyword_lure`: reuses the span's own imagery while asserting something else

Implicit-specific failure modes:
- Choosing a span that is already literal — nothing is implied.
- An option that simply translates the span word-for-word.
- Requiring outside cultural knowledge to decode the metaphor.
