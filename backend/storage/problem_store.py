...
from backend.schemas.storage import ProblemResult, ProblemType, SavedProblemRecord, build_passage_id

logger = logging.getLogger(__name__)


class LocalProblemStore:
    def __init__(self, root_dir: str | Path = "backend/problems") -> None:
    ...
    def save(self, *, request: GenerateRequest, result: ProblemResponse) -> SavedProblemRecord:
        ...
        for attempt_no in range(1, max_attempt + 1):
            ...
            rel_path = (Path("backend/problems") / typed_problem_type / passage_id / filename).as_posix()
