...
from backend.llm.schema import ImplicitDraft
...
from backend.toolkit.text import assert_span_exists, replace_nth, split_sentences
from backend.toolkit.validators import validate_common, validate_implicit_from_original
        ...
        for attempt in range(1 + self.settings.llm_max_retries):
            ...
            try:
                ...
                draft_json = self.llm_client.generate_json(
                    prompt=prompt, schema=ImplicitDraft.model_json_schema(), label="implicit"
                )
