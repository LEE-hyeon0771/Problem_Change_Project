...
from backend.llm.schema import SelfCheckResult
from backend.prompts.loader import render_prompt
from backend.schemas.analysis import CoreferenceCandidate, ParagraphAnalysis, PassageAnalysis
from backend.schemas.base import Choice, GenerateRequest
from backend.toolkit.discourse import find_markers, tag_paragraph_function
from backend.toolkit.labels import choice_labels
from backend.toolkit.text import (
    normalize_text,
    split_paragraphs,
    split_sentences,
...
class BaseAgent(ABC):
    ...
    def analyze(self, passage: str) -> PassageAnalysis:
        emit_progress("analyze", "지문을 분석하고 있습니다...")
    ...
    def _try_llm_generate(
        ...
        *,
        ...
        try:
            ...
            emit_progress("generating", "LLM이 문항을 만들고 있습니다...", prompt=name)
            prompt = render_prompt(name, **context)
            schema = response_model.model_json_schema()
            generated = self.llm_client.generate_json(prompt=prompt, schema=schema, label=name)
            generated.setdefault("type", self.problem_type)
            emit_progress("validating", "생성 결과를 검증하고 있습니다...")
    ...
    def _run_self_check(self, problem: BaseModel) -> bool:
        ...
        try:
            result = self.llm_client.generate_json(
                prompt=prompt, schema=SelfCheckResult.model_json_schema(), label="self_check"
            )
    ...
    async def agenerate(self, request: GenerateRequest) -> BaseModel:
        ...
        # run_in_executor는 contextvars를 자동으로 넘기지 않습니다. 명시적으로 복사해야
        # 워커 스레드에서 호출하는 LLM의 토큰 사용량이 요청 recorder에 기록됩니다.
        ctx = contextvars.copy_context()
        with ThreadPoolExecutor(max_workers=1) as executor:
            return await loop.run_in_executor(executor, lambda: ctx.run(self.generate, request))
