...
from backend.llm.schema import BlankChoiceRepair, BlankDraft, BlankUniquenessCheckResult
...
from backend.toolkit.difficulty import blank_span_type
from backend.toolkit.labels import choice_labels
from backend.toolkit.render import render_blank
from backend.toolkit.text import split_sentences
from backend.toolkit.validators import validate_blank, validate_blank_from_original, validate_common
...
class BlankAgent(BaseAgent):
        ...
        *,
        ...
        try:
            repaired = self.llm_client.generate_json(
                prompt=prompt, schema=BlankChoiceRepair.model_json_schema(), label="blank_choices_repair"
            )
    ...
    def _try_llm_blank_generate(self, request: GenerateRequest, passage: str, analysis) -> BlankResponse | None:
        ...
        # 최초 1회 + 재시도 N회. provider 내부 재시도와 곱해지므로 여기서도 상한을 둡니다.
        for attempt in range(1 + self.settings.llm_max_retries):
            ...
            try:
                ...
                draft_json = self.llm_client.generate_json(
                    prompt=prompt, schema=BlankDraft.model_json_schema(), label="blank"
                )
